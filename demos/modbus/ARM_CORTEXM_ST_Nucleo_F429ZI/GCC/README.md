These demo programs are targeted towards an ST [Nucleo-F429ZI](https://www.st.com/en/evaluation-tools/nucleo-f429zi.html) board and are configured for building with the [GNU ARM Embedded](https://developer.arm.com/tools-and-software/open-source-software/developer-tools/gnu-toolchain/gnu-rm) toolchain.
...
```bash
cd demos/modbus/ARM_CORTEXM_ST_Nucleo_F429ZI/GCC/serverrtu
make
```

This creates the `NucleoF429ZI_Modbus_ServerRtu.elf` executable in that same directory. This file can be programmed onto the ST Nucleo-F429ZI board, using the on-board ST-Link debugger interface and with the help of the [STM32CubeProgrammer](https://www.st.com/en/development-tools/stm32cubeprog.html) software program.
